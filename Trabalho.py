...
def desenhar_plano(canvas):
    canvas.create_line(250, 0, 250, 500, fill="black", width=2)  # Eixo Y
    canvas.create_line(0, 250, 500, 250, fill="black", width=2)  # Eixo X
    for i in range(0, 500, 50):
        canvas.create_line(i, 245, i, 255, fill="black")  # Marcas no eixo X
        canvas.create_line(245, i, 255, i, fill="black")  # Marcas no eixo Y

# Função para adicionar pontos ao plano
def adicionar_ponto(canvas, x, y):
    ponto = canvas.create_oval(250 + x * 25 - 5, 250 - y * 25 - 5, 250 + x * 25 + 5, 250 - y * 25 + 5, fill="red")
    texto = canvas.create_text(250 + x * 25, 250 - y * 25 - 10, text=f'({x},{y})', fill="black")
...
def desenhar_pontos_linha():
    ...
    for i in range(len(pontos) - 1):
        ...
        for x, y in bresenham(p1[0], p1[1], p2[0], p2[1]):
            ponto = canvas.create_oval(250 + x * 25 - 2, 250 - y * 25 - 2, 250 + x * 25 + 2, 250 - y * 25 + 2, fill="blue")
...
def desenhar_circulo():
    ...
    for x, y in bresenham_circulo(x_center, y_center, radius):
        ponto = canvas.create_oval(250 + x * 25 - 2, 250 - y * 25 - 2, 250 + x * 25 + 2, 250 - y * 25 + 2, fill="green")
        linhas_desenhadas.append(ponto)
...
def desenhar_elipse():
    ...
    for x, y in bresenham_elipse(x_center, y_center, a, b):
        ponto = canvas.create_oval(250 + x * 25 - 2, 250 - y * 25 - 2, 250 + x * 25 + 2, 250 - y * 25 + 2, fill="purple")
        linhas_desenhadas.append(ponto)
...
def desenhar_bezier():
    ...
    for i in range(len(pontos) - 2):
        ...
        for x, y in bezier2(p1, p2, p3):
            ponto = canvas.create_oval(250 + x * 25 - 2, 250 - y * 25 - 2, 250 + x * 25 + 2, 250 - y * 25 + 2, fill="orange")
            linhas_desenhadas.append(ponto)
...
def desenhar_poligono():
    ...
    for i in range(len(pontos)):
        ...
        for x, y in bresenham(p1[0], p1[1], p2[0], p2[1]):
            ponto = canvas.create_oval(250 + x * 25 - 2, 250 - y * 25 - 2, 250 + x * 25 + 2, 250 - y * 25 + 2, fill="cyan")
            linhas_desenhadas.append(ponto)
    ...
    if ponto_dentro_poligono((x, y), poligono):
        ...
        for i in range(-12, 13):
            for j in range(-12, 13):
                if ponto_dentro_poligono((i, j), poligono):
                    ponto = canvas.create_oval(250 + i * 25 - 1, 250 - j * 25 - 1, 250 + i * 25 + 1, 250 - j * 25 + 1, fill=fill_color)
                    pontos_preenchidos.append(ponto)  # Armazena a referência do ponto preenchido
    else:
        # Desenhar pontos fora do polígono
        for i in range(-12, 13):
            for j in range(-12, 13):
                if not ponto_dentro_poligono((i, j), poligono):
                    ponto = canvas.create_oval(250 + i * 25 - 1, 250 - j * 25 - 1, 250 + i * 25 + 1, 250 - j * 25 + 1, fill=fill_color)
                    pontos_preenchidos.append(ponto)  # Armazena a referência do ponto preenchido
...
adicionar_ponto(canvas, -4, -5)
adicionar_ponto(canvas, 0, 0)
adicionar_ponto(canvas, 2, 3)
adicionar_ponto(canvas, -7, 7)
